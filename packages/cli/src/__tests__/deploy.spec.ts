  ...
  describe('#getProjectConfig', () => {
    ...
    it('should add html webpack plugin', () => {
      getCustomConfigMocked.mockReturnValueOnce({
        entries: { a: { entry: './a.js', template: './a.html' }, b: { entry: './b.js', template: './b.html' } },
      });
      const actual = getProjectConfig({ name: 'test-only' });
      expect(actual.plugins).toEqual(expect.arrayContaining([expect.any(HtmlWebpackPlugin), expect.any(HtmlWebpackPlugin)]));
      expect(getCustomConfigMocked).toBeCalledTimes(1);
